...
final class LobbyFlowCoordinator: FlowCoordinator {
    ...
    init(
        ...
        self.viewFactories = viewFactories ?? [LobbyFlowViewFactory(dependencyProvider: dependencyProvider)]
        self.coordinatorFactories = coordinatorFactories ?? []
    }

    func start(animated: Bool) {
        let initialRoute = LobbyRoute.lobby
